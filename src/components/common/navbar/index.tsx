...
import { useGetAccounts } from "../../../hooks/common/useGetAccounts";
import { useUser } from "@auth0/nextjs-auth0/client";
import { useGetAccount } from "../../../hooks/common/useGetAccount";
					...
					<DropdownMenu>
						{userLinks.map((link, index) => (
							<DropdownItem key={index}>
								{link.name === 'Logout' ? <a href={link.url}>{link.name}</a> : <Link href={user ? link.url : ''}>{link.name}</Link>}
							</DropdownItem>
						))}
					</DropdownMenu>
				</Dropdown>
